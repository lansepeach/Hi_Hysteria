...
`hihy 9` 会备份原配置、ACL、脚本证书目录和防火墙所有权记录。新配置失败时会重建旧规则、恢复文件，并检查原本运行的服务；原本停止的服务保持停止。

自 1.0.25 起，录入期间原服务继续运行，录入完成后才停服应用。保留现有 ACL、出站顺序及凭据、DNS resolver 和嗅探设置；只有向导控制的 `reject(all, udp/443)` 规则随 HTTP/3 开关增删。外部绝对路径 ACL 或 inline 规则会复制到脚本托管的 ACL 文件，外部源文件保持不变；相对 ACL 路径需先改为绝对路径。自定义认证不会被完整向导覆盖。

成功后的旧配置保存在 `result/snapshots/reconfigure.*`，可按需保留或手动清理。安装/卸载、核心更新、重配置、出口/ACL/监控修改、服务启停和证书管理使用同一把锁，防止并发覆盖。

如果恢复不完整，命令返回失败并输出 `result/reconfigure.*` 备份路径。请保留该目录，依据日志处理服务或防火墙问题；脚本不会在恢复失败后删除这份备份。备份可能含密码和证书私钥，请勿公开上传。
